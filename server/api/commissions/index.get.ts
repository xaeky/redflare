export default defineEventHandler(async (event) => {
  ...
  const filters: CommissionFilterOptions = JSON.parse(
    String(getQuery(event).filters || '{}'),
  );
  sort_by ||= 'created_at';
  sort_order =
    sort_order === '1' || sort_order === '-1' ? Number(sort_order) : 1;
  const sort =
    sort_by && sort_order
      ? { by: String(sort_by), order: Number(sort_order) as 1 | -1 }
      : undefined;
