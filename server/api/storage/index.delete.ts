export default defineEventHandler(async (event) => {
  const body = await readBody<{
    id: string;
    fileType: 'avatarFile';
    bucketType: 'temp' | 'default';
  }>(event);
  if (!body?.id)
    throw createError({ status: 400, statusText: 'No object ID provided' });
  logger.debug('Received request to delete file', {
    fileId: body.id,
  ...
  try {
    ...
    throw createError({
      status: 500,
      statusText: error.message || 'Failed to delete file',
    });
  }
});
