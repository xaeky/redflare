...
export default defineEventHandler(async (event) => {
  ...
  const sanitizedWhitelistFields:Array<keyof typeof data> = ['_id', 'characters', 'created_at', 'updated_at', 'status', 'public_note', 'flags'];
  const sanitizedPublicData = _.pick(data, sanitizedWhitelistFields);
  // Return commission data
  console.log('sanitizedPublicData', sanitizedPublicData);
