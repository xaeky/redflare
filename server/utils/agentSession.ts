import { createError, type H3Event, type useSession } from 'h3';
...
export async function needAuth(event: EventUserSession) {
  ...
  if (!_.get(userSession, 'user.id'))
    throw createError({ status: 401, message: 'Your user has not an ID.' });
  ...
  const cachedSession = await cache.get<boolean>(cacheKey);
  if (!cachedSession) {
...
export async function getPermissions(
  event: EventUserSession,
  useTrustedSession: boolean = true,
) {
  ...
  const session = useTrustedSession
    ? await needAuth(event)
    : await getUserSession(event);
  if (!session.user)
    throw createError({
      status: 500,
      message: 'Could not read user from the session key.',
    });
  if (!useTrustedSession)
  ...
  const account = await useAgentAccountsModel().getById(
...
export async function updateCurrentUserPassword(
  ...
  if (!isNewValid.success)
    throw createError({
      status: 400,
      message: isNewValid.error.errors[0]?.message,
    });
...
export async function updateCurrentUserSetting(
  event: EventUserSession,
  settingKey: keyof AgentUserSettings,
  settingValue: any,
) {
...
export async function replaceCurrentUserSettings(
  event: EventUserSession,
  newSettings: Partial<AgentUserSettings>,
) {
  const userSettings = await initCurrentUserSettings(event);
...
export async function getCurrentUserSetting(
  event: EventUserSession,
  settingKey: keyof AgentUserSettings,
) {
  const userSettings = await initCurrentUserSettings(event);
...
export async function setWebauthnChallenge(
  event: EventUserSession,
  { challenge, attemptId }: { challenge: string; attemptId: string },
) {
  await setUserSession(event as H3Event, {
...
export async function getWebauthnChallenge(
  ...
  if (!challenge)
    throw createError({
      status: 400,
      message: 'No WebAuthn challenge found for the given attempt ID.',
    });
  return challenge;
}
 ...
 */
export async function recordConfirmationIntent(
  event: EventUserSession,
  newIntent: AgentConfirmIntentCreateOptions,
) {
  ...
  logger
    .withTag('confirmation')
    .debug(
      `Recorded confirmation intent for action "${newIntent.action}" with token: ${opaqueToken}`,
    );
  return opaqueToken;
}
 ...
 */
export async function approveConfirmationIntent(token: string) {
  const confirmationCache = useConfirmationCache();
  ...
  if (!intent)
    throw createError({
      status: 400,
      message: 'No confirmation intent found for the given token.',
    });
  if (new Date(intent.expiresAt) <= new Date()) {
    await confirmationCache.remove(cacheKey);
    throw createError({
      status: 400,
      message: 'The confirmation intent has expired.',
    });
  }
 ...
 */
export async function needConfirmationIntent(
  event: H3Event,
  action: string,
  ...
  if (token) {
    const cacheKey = `confirmation-intent-${token}`;
    const intent = await confirmationCache.get<AgentConfirmIntent>(
      `confirmation-intent-${token}`,
    );
  ...
  throw createError({
    ...
    data: { confirmationToken: opaqueToken },
  });
}
