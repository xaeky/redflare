...
import { createStorage, type StorageMeta } from 'unstorage';
...
export const useStorageBucket = (
  bucketType: 'default' | 'temp' = 'default',
) => {
...
export const bucketGetSignedUploadUrl = async ({
  destinationPath,
  fileName,
  expiresInSeconds = 3600,
  contentType = 'application/octet-stream',
}: BucketSignedUploadUrlOptions) => {
  const client = useStorageClient();
  const endpoint = getEndpoint();

  const url = new URL(
    `${endpoint}/${getBucketName('temp')}/${destinationPath}`,
  );
...
export const bucketGetSignedDownloadUrl = async (
  destinationPath: string,
  expiresInSeconds = 3600,
) => {
  const client = useStorageClient();
  ...
  if (!exists) {
    logger
      .withTag('storage')
      .warn(
        'File not found in bucket when attempting to generate signed download URL',
        { destinationPath },
      );
  ...
  const url = new URL(
    `${endpoint}/${getBucketName('default')}/${destinationPath}`,
  );
  url.searchParams.append('X-Amz-Expires', expiresInSeconds.toString());
  url.searchParams.append(
    'response-content-disposition',
    buildContentDisposition((meta['original-name'] as string) || 'unknown'),
  );
...
export const bucketMoveFileToPermanent = async (filePath: string) => {
  ...
  if (await permanentBucket.hasItem(destinationPath)) {
    logger
      .withTag('storage')
      .warn('File already exists in permanent bucket, skipping move', {
        destinationPath,
      });
    return { id: filePath };
  }
  if (!(await tempBucket.hasItem(destinationPath)))
    throw createError({
      status: 404,
      statusText: 'File not found in temp bucket',
    });
  ...
  headers.set(
    ...
    (tempMeta['original-name'] as string) || 'unknown',
  );
  headers.set(
    'x-amz-copy-source',
    `/${getBucketName('temp')}/${destinationPath}`,
  );
  headers.set('x-amz-metadata-directive', 'REPLACE');
  await client.fetch(copyUrl.toString(), { method: 'PUT', headers });
  await tempBucket
    .removeItem(destinationPath, { removeMeta: true })
    .catch((error) => {
      logger
        .withTag('storage')
        .warn(
          'Failed to remove file from temp bucket after moving to permanent bucket',
          { filePath, error },
        );
    });
...
export const bucketDeleteFile = async (
  destinationPath: string,
  bucketType: 'default' | 'temp' = 'default',
) => {
  ...
  bucket.removeItem(destinationPath, { removeMeta: true }).catch((error) => {
    throw createError({
      status: 500,
      statusText: `Failed to delete file from bucket: ${error.message}`,
    });
  });
  return true;
};

export const bucketFileExists = async (
  destinationPath: string,
  bucketType: 'default' | 'temp' = 'default',
): Promise<boolean> => {
  const bucket = useStorageBucket(bucketType);
...
export const bucketFilesExists = async (
...
const toAttachmentMeta = (
  ...
  filetype: getContentTypeByExtension(
    (meta['original-name'] as string) || 'unknown',
  ),
  size,
});

export async function bucketFileMetadata(
  fileId: string,
  bucketType?: 'default' | 'temp',
  sanitized?: false,
): Promise<StorageMeta>;
export async function bucketFileMetadata(
  ...
  bucketType: 'default' | 'temp',
  sanitized: true,
): Promise<CommissionCharacterAttachmentRaw>;
export async function bucketFileMetadata(
  fileId: string,
  bucketType: 'default' | 'temp' = 'default',
  sanitized: boolean = false,
): Promise<StorageMeta | CommissionCharacterAttachmentRaw> {
  const bucket = useStorageBucket(bucketType);
...
export async function bucketFilesMetadata(
  fileIds: string[],
  bucketType?: 'default' | 'temp',
  sanitized?: false,
): Promise<Record<string, StorageMeta>>;
export async function bucketFilesMetadata(
  fileIds: string[],
  bucketType: 'default' | 'temp',
  sanitized: true,
): Promise<Record<string, CommissionCharacterAttachmentRaw>>;
export async function bucketFilesMetadata(
  fileIds: string[],
  bucketType: 'default' | 'temp' = 'default',
  sanitized: boolean = false,
): Promise<Record<string, StorageMeta | CommissionCharacterAttachmentRaw>> {
  const bucket = useStorageBucket(bucketType);
  const metadataMap: Record<
    string,
    StorageMeta | CommissionCharacterAttachmentRaw
  > = {};
