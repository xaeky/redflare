...
const getOneById = async (
  ...
  const commissionPipeline: Record<string, any>[] = [
    ...
    { $unwind: { path: '$characters', preserveNullAndEmptyArrays: true } },
    {
      $lookup: {
        ...
        pipeline: [{ $match: { $expr: { $eq: ['$_id', '$$baseId'] } } }],
        as: 'baseObj',
      },
    },
    {
  ...
  if (viewAs === 'anon') {
    ...
    commissionPipeline.push({
      $addFields: {
        'locked_fields.characters_count': { $size: '$characters' },
      },
    });
  }
  if (viewAs === 'customer') {
    commissionPipeline.push(
      {
        $addFields: {
          payments_obj_ids: {
            $map: { input: '$payments', as: 'p', in: { $toObjectId: '$$p' } },
          },
        },
      },
      {
  ...
  const commission = await collection
    .aggregate<CommissionBaseRaw>(commissionPipeline)
    .next();
  if (!commission || _.isEmpty(commission)) return null;
  const customer = await useCustomerModel().getById(
    commission.customer.toString() || '',
  );
  ...
  const fileIds = _.flatMap(commission.characters ?? [], (char) =>
    _.flatMap(
      char.changelog ?? [],
      (changelog) => (changelog.attachments as string[]) ?? [],
    ),
  ).map(
    (attachmentId) =>
      `avatars/${normalizeAttachmentId(attachmentId, commission.schemaVersion)}`,
  );
  let filesDetails: Record<string, CommissionCharacterAttachmentRaw> = {};
  if (fileIds.length > 0)
    filesDetails = await bucketFilesMetadata(fileIds, 'default', true);
  // Rename filesDetails keys, split "avatars/" prefix from each key.
  filesDetails = _.mapKeys(filesDetails, (_, key) =>
    key.replace('avatars/', ''),
  );
...
const updateOne = async (
  commissionId: string,
  requestData: Partial<CommissionUpdate>,
) => {
  const collection = await useMongoCollection('commissions');
  const newData: Partial<CommissionUpdate> = {
    ...requestData,
    ...(requestData.customer
      ? { customer: new ObjectId(requestData.customer as string) }
      : {}),
      ...
      ? {
          characters: requestData.characters.map((c) => {
            ...
            sanitizedCharacter.changelog = (c.changelog ?? []).map((log) => ({
              ...log,
              attachments: (log.attachments ?? []).map((id) =>
...
const removeTransactionFromOne = async (
  commissionId: string,
  paymentId: string,
) => {
  const collection = await useMongoCollection('commissions');
  ...
  if (!commission)
    throw createError({ status: 404, statusText: 'Commission not found' });
  const updatedPayments = (commission.payments || []).filter(
    (pId: string) => pId !== paymentId,
  );
  const result = await collection.updateOne(
    { _id: new ObjectId(commissionId) },
    {
...
const addTransactionToOne = async (commissionId: string, paymentId: string) => {
  const collection = await useMongoCollection('commissions');
  const transactionCollection = await useMongoCollection(
  ...
  if (!transaction)
    throw createError({
      status: 404,
      statusText: 'Payment transaction not found',
    });
  const commission = await collection.findOne({
    _id: new ObjectId(commissionId),
  });
  ...
  if (commission.payments?.includes(paymentId)) {
    throw createError({
      status: 400,
      statusText: 'Payment transaction already linked to this commission',
    });
  }
  const updatedPayments = Array.from(
    new Set([...(commission.payments || []), paymentId]),
  );
  const result = await collection.updateOne(
    { _id: new ObjectId(commissionId) },
    {
...
const checkOwnershipFromOne = async (
  commissionId: string,
  evalCustomerId: string,
) => {
...
const migrateCommissions = async (): Promise<{ migrated: number }> => {
  ...
  const cursor = collection.find({
    schemaVersion: { $not: { $gte: CURRENT_SCHEMA_VERSION } },
  });
  ...
  for await (const doc of cursor) {
    const updatedCharacters = (doc.characters ?? []).map((char) => ({
      ...
      changelog: (char.changelog ?? []).map((log) => ({
        ...log,
        attachments: ((log.attachments as string[]) ?? []).map((id) =>
          normalizeAttachmentId(id, doc.schemaVersion),
        ),
      })),
    }));
    bulkOps.push({
