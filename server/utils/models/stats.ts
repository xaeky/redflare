import type { CommissionStatusType } from '~~/shared/enums/Commissions';
...
const getCommissionsCount = async (filter: CommissionCountFilter) => {
  ...
  const now = new Date();
  const start = getPeriodStartOfDay(filter.period);
...
const getNetRevenue = async (filter: RevenueCountFilter) => {
  const collection = await useMongoCollection<PaymentTransaction>(
