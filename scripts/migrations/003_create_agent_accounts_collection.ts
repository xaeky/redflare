...
export async function up(db: Db) {
  ...
  const existingAccounts = await db
    .listCollections({ name: collectionNames.account })
    .toArray();
  if (existingAccounts.length === 0)
    await db.createCollection(collectionNames.account);
  ...
  const existingCreds = await db
    .listCollections({ name: collectionNames.credentials })
    .toArray();
  if (existingCreds.length === 0)
    await db.createCollection(collectionNames.credentials);
}
...
export async function down(db: Db) {
  ...
  const existingAccounts = await db
    ...
    .toArray();
  if (existingAccounts.length > 0)
    await db.collection(collectionNames.account).drop();
  // Drop the agent_credentials collection if it exists
  const existingCreds = await db
    .listCollections({ name: collectionNames.credentials })
    .toArray();
  if (existingCreds.length > 0)
    await db.collection(collectionNames.credentials).drop();
}
