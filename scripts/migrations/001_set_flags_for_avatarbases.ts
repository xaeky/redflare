...
export async function up(db: Db) {
  ...
  await collection.updateMany({ blacklisted: true }, [
...
export async function down(db: Db) {
  ...
  await collection.updateMany({ flags: { $bitsAllSet: FLAGS.Private } }, [
    { $set: { blacklisted: true } },
    { $unset: ['flags'] },
  ]);

  // Where flags does not have FLAGS.Private, set blacklisted to false and remove the flags field
  await collection.updateMany({ flags: { $bitsAllClear: FLAGS.Private } }, [
