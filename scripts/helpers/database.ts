...
type HelperDatabaseMainMenuActions =
  | 'goto:migrations'
  | 'goto:snapshots'
  | 'select:environment'
  | 'exit'
...
async function manageMigration(direction: 'up' | 'down') {
  ...
  if (!client)
    return logger.error(
      'MongoClient is not initialized. Please select an environment first.',
    );
  ...
  const migrationsToRun =
    direction === 'up'
      ...
      : migrationsFiles
          ...
          .reverse()
          .splice(0, 1); // Only rollback the last applied migration

  if (migrationsToRun.length === 0) {
    logger.info(
      `No migrations to ${direction === 'up' ? 'apply' : 'rollback'}.`,
    );
    return;
  }
  ...
  for (const migrationFile of migrationsToRun) {
    thisSpinner.start(
      `${direction === 'up' ? 'Applying' : 'Rolling back'} migration: ${migrationFile}`,
    );
    const migrationModule = await import(
      join(import.meta.dir, '../migrations', migrationFile)
    );
    if (direction === 'up') {
    ...
    thisSpinner.stop(
      `${direction === 'up' ? 'Applied' : 'Rolled back'} migration: ${migrationFile}`,
    );
  }

  logger.success(
    `Successfully ${direction === 'up' ? 'applied' : 'rolled back'} migrations.`,
  );
}
...
async function migrationsMenu() {
  ...
  logger.info(
    `Applied Migrations: ${appliedMigrations.map((m) => m.name).join(', ') || 'None'}`,
  );
...
function getOutputDir(withDate?: boolean) {
  return join(
    PROJECT_ROOT,
    `./.data/db-snapshots/${CURRENT_ENVIRONMENT_NAME}`,
...
async function restoreSnapshot(uri: string, snapshotDir: string) {
  ...
  logger.info(
    `Restoring snapshot from ${fullSnapshotDir} to database ${databaseName}`,
  );
  ...
  const actions: Record<'save' | 'restore', () => Promise<void>> = {
    ...
    restore: async () => {
      ...
      await restoreSnapshot(
        CURRENT_ENVIRONMENT.database.uri,
...
async function selectLocalEnvironment(): Promise<
  string | symbol | null | undefined
> {
  const selectedEnvironmentName = await selectEnvironmentMenu();
  if (isCancel(selectedEnvironmentName) || !selectedEnvironmentName)
    return mainMenu();
...
async function mainMenu() {
  ...
  > = {
    ...
    'select:environment': async () => {
      ...
      CURRENT_ENVIRONMENT_NAME = selectedEnvironmentName as string;
      logger.step(`Selected Environment: ${CURRENT_ENVIRONMENT_NAME}`);
      const selectedEnvironment = await useEnvironment(
