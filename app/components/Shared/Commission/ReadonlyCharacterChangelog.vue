...
const handleAttachmentDownload = async (attachmentId: string) => {
  ...
  try {
    ...
    if (!fetchError.status) {
      toast.add({
        description: 'Network error occurred while preparing download.',
        color: 'error',
      });
      return;
    }
    if (fetchError.status === 403)
      ...
      toast.add({ description: 'File not found.', color: 'error' });
    else
      toast.add({ description: 'Failed to prepare download.', color: 'error' });
  }
};
    ...
    <template #body>
      <div class="rf_shared_ro_character_changelog">
        <UAlert
          ...
          title="Make sure you got everything!"
        >
          <template #description>
            <div class="space-y-4">
              <p>
                Make sure to
                <u
                  >download the {{ avatarBase?.name }} base files from
                  {{ avatarBase?.creator_name }}'s storefront</u
                >, then download the package provided here for the latest
                updates.
              </p>
              <div class="flex flex-wrap gap-2 items-start">
                ...
                >
                  <UButton
                    ...
                    variant="subtle"
                    label="View uploading guide"
                  />
                </ULink>
                <ULink
                  external
                  target="_blank"
                  :to="(avatarBase?.storefront_url as string)"
                >
                  <UButton
                    size="sm"
                    icon="i-lucide-external-link"
        ...
        >
          ...
          <div v-if="entry.attachments?.length" class="release_files">
            <div v-for="attachment in entry.attachments" :key="attachment">
              <button
                type="button"
                class="release_attachment group"
                @click="handleAttachmentDownload(attachment)"
              >
                <div>
                  <div
                    class="bg-neutral-950/50 rounded-full w-12 h-12 flex items-center justify-center group-hover:text-primary group-active:scale-90 duration-100"
                  >
                ...
                <div>
                  <div
                    class="filename"
                    v-text="attachments[attachment]?.filename"
                  />
                  <div class="details">
                    {{ attachments[attachment]?.filetype }}
                    •
                    {{ formatFileSize(attachments[attachment]?.size as number) }}
                  </div>
                </div>
              </button>
            </div>
...
.rf_shared_ro_character_changelog {
  @apply space-y-4;
  .changelog_entry {
    @apply space-y-2 p-4 border border-muted/50 rounded-xl;
  }
  .changelog_title {
