...
import {
  BackofficeCommissionEditSlideover,
...
const copyCommissionLink = (commissionId: string) => {
  ...
  clipboard.copy(link);
  useToast().add({
    title: 'Copied commission link to clipboard.',
    duration: 3000,
    color: 'success',
...
const copyCommissionId = (commissionId: string) => {
  ...
  useToast().add({
    title: 'Copied commission ID to clipboard.',
    duration: 3000,
    color: 'success',
    icon: 'i-lucide-clipboard-check',
  });
...
const columns: TableColumn<DeserializedCommissionWithCustomer>[] = [
  {
    ...
    cell: ({ row }) => {
      ...
      const idDigit = thisCommission._id.substring(
        thisCommission._id.length - 6,
      );
      return h('div', { class: 'flex items-center gap-2' }, [
        h(UButton, {
          onClick: () => copyCommissionLink(thisCommission._id),
        ...
        h(
          UTooltip,
          { text: 'Copy full commission ID', delayDuration: 0 },
          {
            default: () =>
              h(
                'span',
                {
                  onClick: (e: MouseEvent) => {
                    e.stopPropagation();
                    copyCommissionId(thisCommission._id);
                  },
...
function onRowSelect(
  _e: Event,
  row: TableRow<DeserializedCommissionWithCustomer>,
) {
