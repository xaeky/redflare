...
const billingTransactionState = reactive<
  z.infer<typeof billingTransactionSchema>
>({
  total_paid_amount: 0,
...
<template>
  <UModal title="Add billing transaction log" :dismissible="false">
    <template #body>
      <UForm
        ref="formsTransactionCreate"
        :schema="billingTransactionSchema"
        :state="billingTransactionState"
        @submit="billingTransactionFormPostHandle"
        class="space-y-4"
      >
        <div class="grid grid-cols-2 gap-4">
          ...
          >
            <UInput
              type="number"
              v-model="billingTransactionState.total_paid_amount"
            />
          </UFormField>
          <UFormField
          ...
          >
            <UInput
              type="number"
              v-model="billingTransactionState.net_received_amount"
            />
          </UFormField>
        </div>
        <div class="grid grid-cols-2 gap-4">
          <UFormField label="Payment currency" name="payment_currency" required>
            <USelect
              v-model="billingTransactionState.payment_currency"
              :items="billingForm.paymentCurrencyOptions"
              class="w-full"
            />
          </UFormField>
          <UFormField
          ...
          >
            <USelect
              v-model="billingTransactionState.payment_processor"
              :items="billingForm.paymentProcessorOptions"
              class="w-full"
            />
          </UFormField>
        </div>
        <UFormField label="Payment external URL" name="payment_ext_url">
          <UInput
            v-model="billingTransactionState.payment_ext_url"
            class="w-full"
          />
        </UFormField>
        <div class="grid grid-cols-2 gap-4">
          <UFormField
            label="Payment external ID"
            name="payment_ext_id"
            required
          >
            <UInput
              v-model="billingTransactionState.payment_ext_id"
              class="w-full"
            />
          </UFormField>
          <UFormField label="Approved at" name="approved_at" required>
            <UInput
              type="datetime-local"
              v-model="billingTransactionState.approved_at"
              class="w-full"
            />
          </UFormField>
        </div>
        <UFormField label="Internal note" name="internal_note">
          <UInput
            v-model="billingTransactionState.internal_note"
            class="w-full"
          />
        </UFormField>
      </UForm>
    ...
    <template #footer>
      <div class="flex justify-end w-full">
        <UButton
          @click="billingTransactionFormPostHandle"
          :loading="billingTransactionFormBusy"
