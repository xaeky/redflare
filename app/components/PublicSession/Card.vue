...
const hiddenToAnonymous =
  (!props.allowLogin && !isLoggedIn.value) ||
  (!isLoggedIn.value && props.allowLogin) ||
  isLoggedIn.value;
</script>
...
.rf-publicsession-sessioncard {
  ...
  .sc-avatar {
    @apply rounded-full object-cover w-16 h-16;
  }
  .sc-loggedinas {
    @apply text-muted;
  }
  ...
  &.small-card {
    @apply p-4;
    .sc-session-container {
      @apply flex-row justify-between items-center gap-8;
    }
    .sc-avatar {
      @apply w-8 h-8;
    }
