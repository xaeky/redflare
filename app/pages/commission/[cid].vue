...
const commission = await useAPI<PublicSerializedCommission>(
  `/api/public/commissions/${commissionId}`,
);
type AllowedTimelineValues =
  | CommissionStatusType.InSetup
  | CommissionStatusType.NextUp
  | CommissionStatusType.InDevelopment
  | CommissionStatusType.Showtime;
const commissionTimelineAllowedValues: AllowedTimelineValues[] = [
  CommissionStatusType.InSetup,
  CommissionStatusType.NextUp,
  CommissionStatusType.InDevelopment,
  CommissionStatusType.Showtime,
...
const commissionTimeline = ref<TimelineItem[]>([
  {
    ...
    description:
      "We've received your order and we are making the project workspace.",
  ...
  },
  {
    title: 'Showtime',
    description:
      "Your commission was delivered by the artist and it's ready to use!",
    icon: 'i-lucide-package-check',
    value: CommissionStatusType.Showtime.toString(),
  },
]);
const commissionRoutedValues: Record<
  AllowedTimelineValues,
...
> = {
  ...
  [CommissionStatusType.InDevelopment]: [
    CommissionStatusType.InDevelopment,
    CommissionStatusType.AwaitingResponse,
    CommissionStatusType.Maintenance,
  ],
  [CommissionStatusType.Showtime]: [
    CommissionStatusType.Showtime,
    CommissionStatusType.Settled,
  ],
};

// Overlays
const overlay = useOverlay();
const characterChangelogOverlay = overlay.create(
  SharedCommissionReadonlyCharacterChangelog,
);

// Viewport calculations
const isMobile = useMediaQuery('(max-width: 768px)');

function routeCommissionStatus(
...
const commissionRoutedValue = routeCommissionStatus(
  commission.data.status as CommissionStatusType,
);
const commissionRoutedValueString = computed(() =>
  commissionRoutedValue.toString(),
);

function handleCharacterChangelogOpen(
  changelog: CommissionCharacterChangelog[],
  characterIndex: number,
) {
  characterChangelogOverlay.open({
    changelog,
    commission: commission.data._id?.toString() || '',
...
const commissionDescription = commission.data.characters?.length
  ? (commission.data.characters as CommissionCharacterRaw[])
      .map((c) => c.name)
      .join(', ')
...
<template>
  <div class="space-y-4">
    ...
    <div v-if="!isLoggedIn && !agentUser" class="fixed bottom-0 inset-x-0 px-4">
      <div
        class="flex flex-col md:flex-row gap-4 items-center justify-between container mx-auto p-6 bg-muted rounded-xl"
      >
        <p>
          You are viewing this commission as a guest. To view more details,
    ...
    <div v-if="agentUser?.settings.forceAgentView" class="md:p-8">
      <UAlert
        variant="outline"
        color="warning"
        title="Viewing as Commission Owner"
        icon="i-heroicons-eye-16-solid"
      >
        <template #description>
          You are currently viewing this commission as the owner. To see the
          public view, please disable the "Force view as owner in commission
          detail pages" setting in your account settings.
        </template>
      </UAlert>
    </div>
    <div class="flex flex-col lg:flex-row items-stretch space-y-4 md:space-y-0">
      <div class="md:p-8 flex-1 space-y-8">
        <div
          class="flex gap-2 flex-col md:flex-row items-start md:items-center md:justify-between"
        >
          <div>
            <h1>Commission</h1>
        ...
        <div id="commission_sections" class="space-y-4">
          <section
            v-if="commissionTimelineAllowedValues.includes(commissionRoutedValue as AllowedTimelineValues)"
          >
            <UTimeline
              :items="commissionTimeline"
              v-model="commissionRoutedValueString"
              :orientation="isMobile ? 'vertical' : 'horizontal'"
            />
          </section>
          <section v-if="commission.data.characters?.length" class="space-y-4">
            <h2>Characters</h2>
            <div
              class="grid md:grid-cols-2 lg:grid-cols-1 xl:grid-cols-2 gap-8 select-none"
            >
              <div
                v-for="(char, charIndex) in commission.data.characters"
                :key="charIndex"
              ...
              >
                <div class="flex flex-col gap-2">
                  <div class="flex items-center justify-between">
                    ...
                    <UTooltip text="Character's Order ID">
                      <div class="flex items-center gap-2">
                        <UIcon
                          name="i-lucide-hash"
                          class="opacity-50 text-primary-300"
                        />
                        <span
                          class="text-sm font-bold font-mono uppercase select-all"
                        >
                  ...
                  <span
                    ...
                    v-text="(char.base as AvatarBase).name"
                  />
                  <span
                    v-if="char.note?.length"
                    class="text-xs"
                    v-text="char.note"
                  />
                  <ul v-if="char.tasks?.length">
                    <li
                      v-for="(task, taskIndex) in char.tasks"
                      :key="taskIndex"
                      class="flex items-center gap-2"
                    >
                      <UIcon
                        name="i-lucide-loader"
                        class="text-neutral-300 animate-spin"
                        v-if="!task.completed"
                      />
                      <UIcon
          ...
          <section v-if="commission.data.locked_fields">
            <div class="bg-muted p-4 space-y-4 rounded-lg">
              ...
              <div class="space-y-2">
                <div
                  v-if="commission.data.locked_fields.characters_count !== undefined"
                  class="flex items-center gap-2"
                >
                  <UIcon
                  ...
                  <span
                    >Total Characters:
                    {{ commission.data.locked_fields.characters_count }}</span
                  >
                </div>
              </div>
            </div>
      ...
      <div
        class="md:p-8 lg:w-md border-t lg:border-l border-neutral-700 lg:border-neutral-800 pt-4 md:border-t-0 space-y-4"
      >
        <section class="space-y-2">
          <div class="flex items-center gap-2">
            <UIcon name="i-heroicons-clock-16-solid" class="text-primary" />
            <span
              v-text="new Intl.DateTimeFormat('en-US', { dateStyle: 'long' }).format(new Date(commission.data.created_at))"
            />
          </div>
          <div class="flex items-center gap-2">
            ...
            <ULink
              v-if="commission.customer.vrc_id"
              :to="`https://vrchat.com/home/user/${commission.customer.vrc_id}`"
              external
              target="_blank"
            >
              <UButton
        ...
        <section
          class="space-y-4"
          v-if="commission.data.payments && (commission.data.payments as OptionalId<PaymentTransaction>[]).length"
        >
          <h2>Payments</h2>
          <SharedCommissionReadonlyPayments
            :payments="(commission.data.payments as OptionalId<PaymentTransaction>[])"
          />
        </section>
        <section class="space-y-4" v-if="commission.data.public_note?.length">
          <h2>Notes</h2>
