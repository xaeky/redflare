...
const togglePermission = (
  ...
  if (checked) {
    if (!createFormState.permissions.includes(permission))
      createFormState.permissions.push(permission);
  } else {
    createFormState.permissions = createFormState.permissions.filter(
      (p) => p !== permission,
    );
  }
};
...
const createAccount = async () => {
  ...
  try {
    ...
    toast.add({
      ...
      description:
        (error as any).data?.statusText || (error as any).data?.message,
      color: 'error',
    });
  } finally {
  ...
  <div class="space-y-6">
    ...
    >
      <template #displayName-cell="{ row }"
        >{{ row.original.displayName || '—' }}</template
      >
      <template #createdAt-cell="{ row }"
        >{{ new Date(row.original.createdAt).toLocaleDateString() }}</template
      >
    ...
    <div class="max-w-sm space-y-2">
      ...
      <form class="flex flex-col gap-2" @submit.prevent="createAccount">
        <UInput
          v-model="createFormState.username"
          placeholder="Username"
          autocomplete="off"
        />
        <UInput
          v-model="createFormState.displayName"
          placeholder="Display name (optional)"
        />
        <UInput
          v-model="createFormState.password"
          type="password"
