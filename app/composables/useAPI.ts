...
export async function useAPI<T>(
  ...
  if (response.status === 428 && !isConfirmationRetry) {
    ...
    if (!confirmationToken) {
      throw Object.assign(new Error('Confirmation required, but no token was provided by the server.'), {
    ...
    const modal = overlay.create(ModalPasswordConfirmation, { destroyOnClose: true });
    return await new Promise<T>((resolve, reject) => {
      modal.open({
        ...
        onCancelled: () => {
          reject(Object.assign(new Error('User cancelled the confirmation process.'), {
            status: 428,
            data: response._data
          }));
        }
  ...
  if (!response.ok) {
    throw Object.assign(new Error((response._data as { message?: string } | undefined)?.message || 'API request failed'), {
      status: response.status,
      data: response._data
    });
  }
