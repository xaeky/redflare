...
test('Anon user is able to see any commission', async ({ page, goto, request }) => {
  ...
  const randomCommissionResponse = await request.get('/api/test/public/commissions/findAnyOne');
  const randomCommissionId = (await randomCommissionResponse.json())._id;
  const res = await page.request.get(`/api/public/commissions/${randomCommissionId}`);
