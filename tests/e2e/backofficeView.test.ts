...
test.describe.serial('Commission management with new customer', () => {
  ...
  test('Agent is able to create customers', async ({ page }) => {
    ...
    const res = await page.request.post('/api/customers', {
      data: { name: testState.createdCustomerName, note: 'E2E Test Customer' }
    });
  ...
  test('Agent is able to create commissions with the generated customer', async ({ page }) => {
    await claimSession(page, 'agent');
  ...
  test('Agent should not be able to delete customers linked to commissions', async ({ page }) => {
    await claimSession(page, 'agent');
    const res = await page.request.delete(`/api/customers/${testState.customerId}`);
