...
export function getAllAuditCategoriesValues(
  as: 'number[]' | 'SelectItem[]' = 'number[]',
): number[] | SelectItem[] {
  ...
  if (as === 'SelectItem[]')
    return categories.map((value) => ({
      label: humanizeAuditCategory(value),
      value,
    })) as SelectItem[];
  return categories;
}
...
export function iconizeAuditCategory(category: AuditCategory): string {
  const categoryMap: Record<AuditCategory, string> = {
    ...
    [AuditCategory.DownloadAttachment]:
...
export function getAllAuditActionsValues(
  as: 'number[]' | 'SelectItem[]' = 'number[]',
): number[] | SelectItem[] {
